...
public class TripleMiningXPEvent extends Event {

    private int secondsLeft;
    private int taskId;
    private BossBar bossBar;

    public TripleMiningXPEvent() {
        super("Triple Mining XP", Rarity.UNCOMMON, "TRIPLE_MINING_XP");
    }

    @Override
    public void start(){

        Component c = Cavelet.miniMessage.deserialize("<#19b6e6>Event! Triple Mining XP for 5 minutes.");

        Boosters.miningBooster = 3.0;
        this.secondsLeft = 300;

        this.bossBar = BossBar.bossBar(Cavelet.miniMessage.deserialize("<aqua>Triple Mining XP"), 1F, BossBar.Color.BLUE, BossBar.Overlay.PROGRESS);
        taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(Cavelet.getPlugin(Cavelet.class), () -> {
            ...
            this.bossBar.name(Cavelet.miniMessage.deserialize("<aqua>Triple Mining XP ("+mins+":"+secs+")"));
        ...
        final Title title = Title.title(Cavelet.miniMessage.deserialize("<aqua><bold>Triple Mining XP"), Cavelet.miniMessage.deserialize("<#9eb5db>All players will gain <Aqua>3x Mining XP<#9eb5db>."));
    ...
    public void end(boolean forced){
        Bukkit.getOnlinePlayers().forEach((p) -> {
            Component m = Cavelet.miniMessage.deserialize("<#19b6e6>Even ended! Triple Mining XP event has ended.");
            MProfile mp = Cavelet.cachedMPlayers.get(p.getUniqueId());
            if(mp.getCurrentBossBar()==this.bossBar){
    ...
    public void showEventInfo(Player player){
        Component c = Cavelet.miniMessage.deserialize("<#9eb5db>Current event: Triple Mining XP (All players have a 3x mining XP multiplier)");
