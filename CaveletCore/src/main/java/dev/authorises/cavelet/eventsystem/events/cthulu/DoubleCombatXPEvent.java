...
public class DoubleCombatXPEvent extends Event {

    private int secondsLeft;
    private int taskId;
    private BossBar bossBar;

    public DoubleCombatXPEvent() {
        super("Triple Combat XP", Rarity.UNCOMMON, "DOUBLE_COMBAT_XP");
    }

    @Override
    public void start(){

        Component c = Cavelet.miniMessage.deserialize("<#19b6e6>Event! Double Combat XP for 10 minutes.");

        Boosters.combatBooster = 2.0;
        this.secondsLeft = 600;

        this.bossBar = BossBar.bossBar(Cavelet.miniMessage.deserialize("<red>Double Combat XP"), 1F, BossBar.Color.RED, BossBar.Overlay.PROGRESS);
        taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(Cavelet.getPlugin(Cavelet.class), () -> {
            ...
            this.bossBar.name(Cavelet.miniMessage.deserialize("<red>Double Combat XP ("+mins+":"+secs+")"));
        ...
        final Title title = Title.title(Cavelet.miniMessage.deserialize("<red><bold>Double Combat XP"), Cavelet.miniMessage.deserialize("<#9eb5db>All players will gain <red>2x Combat XP<#9eb5db>."));
    ...
    public void end(boolean forced){
        Bukkit.getOnlinePlayers().forEach((p) -> {
            Component m = Cavelet.miniMessage.deserialize("<#19b6e6>Even ended! Double Combat XP event has ended.");
            MProfile mp = Cavelet.cachedMPlayers.get(p.getUniqueId());
            if(mp.getCurrentBossBar()==this.bossBar){
    ...
    public void showEventInfo(Player player){
        Component c = Cavelet.miniMessage.deserialize("<#9eb5db>Current event: Double Combat XP (All players have a 2x Combat XP multiplier)");
