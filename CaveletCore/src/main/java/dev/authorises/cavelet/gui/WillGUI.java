...
public class WillGUI {
    public WillGUI(Player p){
        ChestGui gui = new ChestGui(6, ColorUtils.format("&cWill"));
        Integer x = 0;
        PaginatedPane pages = new PaginatedPane(0, 0, 9, 5);
        ArrayList<GuiItem> guiItems = new ArrayList<>();
        gui.setOnGlobalClick(event -> event.setCancelled(true));
        for(CustomItemShop shop : Cavelet.willShop.getShops()){
        ...
        for(Buyable b : Cavelet.buyableManager.getBuyables()){
            if(b instanceof ItemStackShop){
                ...
                if(add) {
                    if (Arrays.stream(((ItemStackShop) b).getBuyableFroms()).toList().contains(BuyableFrom.WILL)) {
        ...
        navigation.addItem(new GuiItem(new ItemBuilder(Material.WHITE_STAINED_GLASS_PANE)
                .setName(ColorUtils.format("&dReload")).toItemStack(), event -> {
            event.setCancelled(true);
            p.closeInventory();
            new WillGUI(p);
        }), 4, 0);
