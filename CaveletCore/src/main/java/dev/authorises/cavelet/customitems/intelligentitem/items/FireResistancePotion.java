...
public class FireResistancePotion extends IntelligentItem {

    public FireResistancePotion(){
        setRarity(Rarity.UNCOMMON);
        setId("FIRE_RESISTANCE_POTION");
        setDisplayName("Fire Resistance Potion");
        setMaterial(Material.POTION);
        setDescription("Gives Fire Resistance effect for 5:00");
        setPossibleEnchants();
        setPossibleForges();
        setStackable(false);
        setEnchantable(true);
...
;
    ...
    public void consume(PlayerItemConsumeEvent event, Player player) {
        ...
        try {
            ...
            player.sendMessage("An error occured using your fire resistance potion.");
        }
