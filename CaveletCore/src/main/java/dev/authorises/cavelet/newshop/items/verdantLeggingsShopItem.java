...
public class verdantLeggingsShopItem implements ShopItem {

    @Override
    public @NotNull String displayName() {
        return "Verdant Leggings";
    }
    ...
    public @NotNull ItemStack getDispayItem() {
        return new ItemBuilder(new CItem(Cavelet.customItemsManager.getItemById("VERDANT_LEGGINGS")).getItemStack())
                .setComponentName(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>Verdant Leggings"))
    ...
    public void purchase(Player p, double amt) {
        ...
        if(cost1<=mp.getBalance() && cost2<=mp.getDarkSouls()){
            if(InventoryUtil.freeSlots(p)>=slots){
                ...
                while (amt > 0){
                    CItem cItem = new CItem(Cavelet.customItemsManager.getItemById("VERDANT_LEGGINGS"));
