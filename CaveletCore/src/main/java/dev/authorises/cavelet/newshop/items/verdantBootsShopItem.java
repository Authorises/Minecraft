...
public class verdantBootsShopItem implements ShopItem {

    @Override
    public @NotNull String displayName() {
        return "Verdant Boots";
    }
    ...
    public @NotNull ItemStack getDispayItem() {
        return new ItemBuilder(new CItem(Cavelet.customItemsManager.getItemById("VERDANT_BOOTS")).getItemStack())
                .setComponentName(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>Verdant Boots"))
    ...
    public void purchase(Player p, double amt) {
        ...
        if(cost1<=mp.getBalance() && cost2<=mp.getDarkSouls()){
            if(InventoryUtil.freeSlots(p)>=slots){
                ...
                while (amt > 0){
                    CItem cItem = new CItem(Cavelet.customItemsManager.getItemById("VERDANT_BOOTS"));
