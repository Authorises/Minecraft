...
public class sheepSpawnerShopItem implements ShopItem {
    ...
    public @NotNull ItemStack getDispayItem() {
        return new ItemBuilder(Material.SPAWNER)
                .setComponentName(Cavelet.miniMessage.deserialize("<!italic><#ffffff>Sheep Spawner"))
                ...
                .addComponentLoreLine(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>- <#19b6e6>$100,000"))
                .toItemStack();
    }
    ...
    public void purchase(Player p, double amt) {
        ...
        if(cost1<=mp.getBalance() && cost2<=mp.getDarkSouls()){
            if(InventoryUtil.freeSlots(p)>=slots){
                ...
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "ws give -s "+p.getName()+" spawner sheep 1");
            }else{
