...
public class factionGatewayShopItem implements ShopItem {

    @Override
    public @NotNull String displayName() {
        return "&6Faction Gateway";
    }
    ...
    public @NotNull ItemStack getDispayItem() {
        return new ItemBuilder(new CItem(Cavelet.customItemsManager.getItemById("FACTION_GATEWAY")).getItemStack())
                .setComponentName(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>Faction Gateway"))
                ...
                .addComponentLoreLine(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>- <#FF55FF>1,000 Dark Souls"))
                .addComponentLoreLine(Cavelet.miniMessage.deserialize("<!italic><#9eb5db>- <#19b6e6>$150,000"))
                .toItemStack();
    }
    ...
    public void purchase(Player p, double amt) {
        ...
        double cost1 = amt*150000D;
        double cost2 = amt*1000;
        if(cost1<=mp.getBalance() && cost2<=mp.getDarkSouls()){
            if(InventoryUtil.freeSlots(p)>=slots){
                ...
                while (amt > 0){
                    CItem cItem = new CItem(Cavelet.customItemsManager.getItemById("FACTION_GATEWAY"));
